...
function render_timeline(this)
	...
	if state.ab_loop_a and state.ab_loop_b then
		...
		local rax = bax + this.width * (range_start / state.duration)
		local rbx = bax + this.width * (range_end / state.duration)
		ass:new_event()
			...
			local draw_chapter = function (time)
				...
				if dots then
					...
					ass:bezier_curve(
						...
						chapter_x + chapter_half_size + options.chapters_size - 4, chapter_y - bezier_stretch,
						chapter_x + chapter_half_size + options.chapters_size - 4, chapter_y
					)
					ass:bezier_curve(
						chapter_x + chapter_half_size + options.chapters_size - 4, chapter_y + bezier_stretch,
						chapter_x - chapter_half_size, chapter_y + bezier_stretch,
					...
					ass:rect_cw(math.max(last_chapter, chapter_x), chapter_y - chapter_half_size, chapter_x + options.chapters_size, chapter_y + chapter_half_size)
					last_chapter = chapter_x + options.chapters_size
				end
	...
	if (this.proximity_raw == 0 or this.pressed) and not (elements.speed and elements.speed.dragging) then
		...
		ass:new_event()
		ass:append('{\\blur0\\bord2\\shad0\\1c&H'..options.color_background_text..'\\3c&H262626\\fn'..config.font..'\\fs'..this.font_size..bold_tag..'')
...
function render_top_bar(this)
	...
	if options.top_bar_title and state.media_title then
		...
		ass:new_event()
		ass:append('{\\q2\\blur0\\bord2\\shad0\\1c&HFFFFFF\\3c&H262626\\fn'..config.font..'\\fs'..this.font_size..bold_tag..'\\clip('..clip_coordinates..')')
